...
namespace GameOfLife
{
    ...
    public partial class Form1 : Form
    {
        ...
        private void createMatrix(int rows, int cols)
        {
            ...
            //This matrix contains the second space
            second_space = new uint[cols, rows];
            scrollBox();
        }
        ...
        private void PBAutomataSimulator_Paint(object sender, PaintEventArgs e)
        {
            ...
            if (Helper.getSpaceType(comboSpace.Text) == Helper.NORMAL)
                paintNormalSpace(graphics);
            else
            {
                if (TAO <= 0)
                {
                    paintMemorySpace(graphics);
        ...
        public void paintMemorySpace(Graphics graphics) {
            int x_size = second_space.GetLength(0);
            int y_size = second_space.GetLength(1);
            total_cells = x_size * y_size;
            ...
            for (int row = 0; row < x_size; row++)
            {
                for (int col = 0; col < y_size; col++)
                {
                    uint current_cell = second_space[row, col];
                    switch (Helper.getRule(comboRuleMem.Text)) {
                        case Helper.MAJORITY:
                            current_cell = Helper.getMajority(second_space[row, col], (int)(NumericGenMem.Value)) ;
                            drawCell(graphics, current_cell, row, col);
                        break;

                        case Helper.MINORITY:
                            current_cell = Helper.getMinority(second_space[row, col], (int)(NumericGenMem.Value));
                            drawCell(graphics, current_cell, row, col);
                            break;
                     
                        default:
                            current_cell = Helper.getParity(second_space[row, col], (int)(NumericGenMem.Value));
                            drawCell(graphics, current_cell, row, col);
                            break;
                    }
                    matrix[row, col] = (current_cell == 1) ? ALIVE : DEAD;
                    second_space[row, col] = 0;
                }
            }
            for (int y = 0; y < y_size; y++)
            {
        ...
        public void drawCell(Graphics graphics, uint state, int row, int col) {
            if (state == 1)
                graphics.FillRectangle(alive, row * cellArea, col * cellArea, cellArea, cellArea);
            else
                graphics.FillRectangle(dead, row * cellArea, col * cellArea, cellArea, cellArea);
        }
        /// <summary>
        ...
        private uint[,] nextGeneration(uint[,] p_matrix)
        {
            ...
            for (int row = 0; row < p_matrix.GetLength(0); row++)
            {
                ...
                for (int col = 0; col < p_matrix.GetLength(1); col++)
                {
                    ...
                    //Counting the ones in our second space
                    if ((new_matrix[row, col] != DEAD) && Helper.getSpaceType(comboSpace.Text) == Helper.MEMORY) {
                        second_space[row, col]++;
                    }
                }
        ...
        private void PBAutomataSimulator_MouseMove(object sender, MouseEventArgs e)
        {
            try
            {
                if (move && index_pattern != 0)
                {
                    ...
                    if (Helper.getSpaceType(comboSpace.Text) == Helper.MEMORY)
                        TAO++;
                    PBAutomataSimulator.Invalidate();
                    move = false;
                    index_pattern = 0;
        ...
        private void NumericGenMem_ValueChanged(object sender, EventArgs e)
        {
            TAO = (int)NumericGenMem.Value - 1;
        }
    }
