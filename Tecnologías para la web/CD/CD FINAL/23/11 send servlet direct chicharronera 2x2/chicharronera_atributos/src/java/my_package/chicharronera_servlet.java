...
public class chicharronera_servlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (A == 0 && B == 0 && C == 0) { //Si todos son cero
                ...
                if (A != 0) {
                    ...
                    if (raiz > 0) {
                        ...
                        out.println("<br></br>");
                        session.setAttribute("clave_atributo_01", "El valor de X1 es " + (-B + Math.sqrt(raiz)) / (2 * A));
                        session.setAttribute("clave_atributo_02", "El valor de X2 es " + (-B - Math.sqrt(raiz)) / (2 * A));
                        ...
                        out.println("<h1>El valor de X1 es " + -B / (2 * A) + " - i" + Math.sqrt(Math.abs(raiz)) / (2 * A) + "</h1>");
                        out.println("<br></br>");
                        session.setAttribute("clave_atributo_01", "El valor de X1 es " + (-B / (2 * A)) + " + i" + (Math.sqrt(Math.abs(raiz)) / (2 * A)));
                        session.setAttribute("clave_atributo_02", "El valor de X2 es " + (-B / (2 * A)) + " - i" + (Math.sqrt(Math.abs(raiz)) / (2 * A)));
                    } else {
                        out.println("<h1>El valor de X1 es " + (-B + Math.sqrt(raiz)) / (2 * A) + "</h1>");
                        out.println("<br></br>");
                        out.println("<h1>El valor de X2 es " + (-B - Math.sqrt(raiz)) / (2 * A) + "</h1>");
                    ...
                    session.setAttribute("clave_atributo_01", "No hay solución para esa ecuación");
                    session.setAttribute("clave_atributo_02", "");
                }
            }
            out.println("</body>");
            out.println("</html>");
            ...
            session.setAttribute("clave_atributo_01", "Eso no es un numero");
            session.setAttribute("clave_atributo_02", "");
        }
