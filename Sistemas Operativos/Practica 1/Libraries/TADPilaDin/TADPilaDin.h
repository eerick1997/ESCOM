...
void Initialize_P(pila *s);			//Inicializar pila (Iniciar una pila para su uso)
void Push(pila *s, p_elemento e);		//Empilar (Introducir un elemento a la pila)
p_elemento Pop (pila *s);				//Desempilar (Extraer un elemento de la pila)
boolean Empty_P(pila *s);				//Vacia (Preguntar si la pila esta vacia)
p_elemento Top(pila *s);				//Tope (Obtener el "elemento" del tope de la pila si extraerlo de la pila)
int Size(pila *s);					//Tamaño de la pila (Obtener el número de elementos en la pila)
void Destroy_P(pila *s);				//Elimina pila (Borra a todos los elementos y a la pila de memoria)
