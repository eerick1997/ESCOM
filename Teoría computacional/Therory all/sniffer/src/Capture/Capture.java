    ...
    public static void selectProtocol(PcapPacket pPacket) throws Exception {
        ...
        if (type > 1500) {
            switch (type) {
                ...
                //ARP
                case 2054: //We need to check this value
                    Arp arp = pPacket.getHeader(new Arp());
                    
                    if(pPacket.hasHeader(arp)){
                        ARP myArp = new ARP(arp);
                    } else {
                        System.out.println("Packet doesn't contain an arp header");
                    }
                    break;
            }
