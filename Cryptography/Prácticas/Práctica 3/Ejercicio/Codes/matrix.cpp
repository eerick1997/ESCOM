...
Matrix getAdjMatrix(Matrix &matrix){
    ...
    for(int i = 0; i < n; i++){
        for(int j = 0; j < n; j++){
            ...
            adj[i][j] = sign * det;//mod( (sign * det) , LAN_SIZ); 
        }
    }
...
/**
 * This function multiplies the inverse matrix of the ciphertext
 * and the matrix of the plaintext
 * @param: (Matrix) matrixInverse
 * @param: (Matrix) matrixPlain
 * @return: (Matrix)
**/
Matrix multiplyMatrix(Matrix matrixPlain, Matrix matrixInverse)
{
    int i,j,k;
    Matrix result(3,vector<lli>(3,0));
    for(i = 0; i < 3; i++)
    {
        for(j = 0; j < 3; j++)
        {
            for(k = 0; k < 3; k++)
            {
                result[i][j] += matrixInverse[i][k]*matrixPlain[k][j];
            }
            result[i][j] = mod(result[i][j], LAN_SIZ);
        }
    }
    return result;
}
