...
void Main(){
    ...
    switch (option)
    {
        ...
        case 4:
        {
            ...
            helper(cipher_text);
            helper(plain_text);
            Matrix matrix(3,vector<lli>(3,0));
            Matrix matrix2(3,vector<lli>(3,0));
            Matrix toDecodeM;
            int k=0;
            do{
                 for(int i = 0; i < 3; i++)
                {
                    for(int j = 0; j < 3; j++)
                    {
                            ...
                            matrix2[i][j] = cipher_text[k];
                            k++;
                        
                    }
                }
                
                invMatrix = getAdjMatrix(matrix2);
                det = mod(determinant(invMatrix), LAN_SIZ);
                inv = inverse(det, LAN_SIZ);
            ...
            k=0;
            //invMatrix=getAdjMatrix(matrix2);
            toDecodeM = escalarMatrix(inv, invMatrix);    
            print(matrix2);
            print(toDecodeM);
            Matrix multiply = multiplyMatrix(matrix,toDecodeM);
